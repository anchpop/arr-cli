...
`arr radarr add '<title>' --requester <theirDiscordId>` — or `arr sonarr add` / `arr sonarr-anime add  --requester <theirDiscordId>` for shows (anime belongs on the anime instance). If the request mentions language, add `--require-subs eng` and/or `--require-audio <lang>`; when they didn't spell it out, if it's a foreign movie or the user has expressed a preference you might still pick a sensible default (e.g. original audio + English subs, plus you can always ask them what their preference is). The command refuses ambiguous titles (disambiguate with `arr <svc> lookup`, re-add with `--tvdb`/`--tmdb`), and its output already answers the request: what got grabbed (it waits up to a minute and bumps the grab to the front of the download queue) or, for a show that's already in the library, its per-season coverage and any warnings. Reply from that output. From there the download-notifier DMs the requester a live progress embed, confirms the item in Jellyfin, and ffprobe-verifies any require-* tags on the finished files — following up is its job, not yours. When `add` prints a `⚠ same title already in radarr` line with someone's Seerr request on that other entry, they most likely picked the wrong search result on the website (Seerr lists the popular new film above the older one). Adding the right film alone leaves their request stuck on "requested" forever and lets the mis-pick download when it releases — run the `arr seerr rebind … --tmdb …` line it prints, which moves the request to the film they meant and removes the file-less entry. Part of the library predates Radarr, so "not in radarr" doesn't mean "not on the server": when `add` finds Jellyfin already serving the film from a folder Radarr doesn't track, it adopts that folder instead (tracked, unmonitored, no search) and prints "already watchable" — tell the requester it's there now; nothing downloads. A plain add there would search and grab a multi-GB "upgrade" over a file they could already watch (the Iron Man remux, 2026-09-03). `arr radarr adopt` lists any such folders and takes them in; it's how new ones (a manual copy into `/data/media/movies`) get tracked. Whenever something is already watchable, the output carries a `watch:` deep link into the Jellyfin web client (`add`, `where`, `jellyfin has` all print it) — pass that link on, so the person lands on the film instead of searching for it. The notifier's ✅ ready DM links its title the same way.

**"Where's my request?" / something didn't arrive.**
`arr where '<title>'` first — it answers the question outright and tells you which of the follow-ups below is even relevant (two entries with the same title come out side by side, with a rebind hint when the request sits on the file-less one; a film Jellyfin serves from a folder Radarr doesn't track says so and points at `arr radarr adopt`). Then, as needed: `arr status '<title>'` — a single match prints per-season gaps and unmanaged-file warnings. `arr seerr unfulfilled` cross-checks every website request against actual disk state. `arr <svc> releases '<title>' --season N` shows what the indexers have and why candidates were rejected (anime indexer searches run long; `--timeout 600`). `arr <svc> history '<title>'` shows grabs and failures. Old obscure content often has only dead releases — after the arr exhausts its candidates, "currently unobtainable" is a real and honest answer.

**The search is slow / the wait reported nothing grabbed.**
`add`/`grab`'s wait narrates the search command it's watching (id + live progress message) and a zero-grab timeout ends with the diagnosis: search still running vs finished empty, plus Prowlarr's indexer-backoff state. "3/4 indexers backed off" means indexer trouble, not scarcity — tell the requester that real reason and check back later rather than re-searching into dead indexers. `arr <svc> searches '<title>'` re-answers "is the search alive and where is it?" any time after (⚠ marks a grinder). Searches run concurrently: alongside a slow per-episode grinder, a narrower `grab <id> --season N` is the productive move — a STARTED command always runs to completion (`arr <svc> cancel` covers queued ones). Transient "database is locked" arr errors retry themselves inside the CLI; no sleep-and-retry loops needed.
...
**"What's the queue looking like?" / the download backlog needs a look.**
`arr queue` (no service) is the whole-picture rollup: total jobs, TB remaining, ETA, per-category/state/quality counts, free space vs. what's queued, and a raw-disc warning. Per-service `arr <svc> queue` is the filterable item list — `--disc`, `--quality NAME`, `--status failed|stuck|paused`, or a title substring narrow it, and `--ids` prints bare queue ids to pipe. **ENCRYPTED jobs are fakes — clear them on sight.** SAB flags password-protected rars with an ENCRYPTED label and pauses them, where they sit forever (a legit release is never passworded; these are junk uploads). `arr queue` warns about them prominently and `arr <svc> queue --encrypted` lists them; fix with `arr <svc> queue-rm --encrypted --blocklist --research --yes` — remove, blocklist so the same fake isn't re-grabbed, and re-search for a real release. This needs no approval; it is pure junk removal. Raw-disc grabs (BR-DISK/ISO/BDMV) are the usual bloat — they don't import or re-encode, so a queue full of them is wasted bytes. Clear them with `arr <svc> queue-rm --disc --blocklist --research --yes` (cancels, blocklists the release, and re-searches for an importable one). Radarr's "Any" profile (what nearly all movies use) is recyclarr-managed since 2026-07-23 as "grab the best release that exists, never junk": BR-DISK/Raw-HD/WORKPRINT are disabled (CAM/TELESYNC/TELECINE sit at the ladder bottom as auto-upgrading placeholders) and TRaSH junk custom formats score -10000, so disc releases can no longer be grabbed — a disc in the queue means the profile regressed (radarr DB re-init) and is worth raising with Andre.

**Someone asks for a book.**
`arr book add '<title|isbn>'` is the whole pipeline: Jellyfin dedup check, edition resolve (Hardcover metadata; a bare ISBN falls back through Bindery's OpenLibrary lookup and auto-picks on an exact ISBN match), a usenet availability probe, Anna's Archive/Libgen release search, download (waits by default — AA slow-partner pace is ~1 MB/s, so a 100MB epub legitimately takes ~20 min; `--no-wait` + `arr book status` to follow up instead), and a Jellyfin visibility confirm. Ambiguous titles list candidates for `--book-id <provider_id>`; `--format epub` pins a format. `arr book status` is the Shelfmark + Bindery in-flight rollup. Two things the command tells you but that bear repeating: **books skip the download-notifier — DM the requester yourself** when it reports "visible in Jellyfin" (web reader at https://watch.beef.baby, or a phone e-reader app like KOReader/Moon+ on the OPDS feed https://watch.beef.baby/opds); and a usenet miss is expected for academic/technical titles (only NZBgeek/Nzb.life carry books), not an error. When the command's own diagnosis isn't enough — mirror rot, disabled sources — the raw Shelfmark API and its failure playbook are in `references/shelfmark-book-downloads.md`.
...
`arr <svc> coverage '<show>' --tracks` reads the actual files (embedded streams + sidecars) per season. On main sonarr and radarr, Bazarr fetches missing subs (`--fix-subs`, or `arr bazarr search --series/--movie`); check `arr bazarr status` when nothing arrives — a throttled/expired provider is the usual cause. The anime instance is not Bazarr-covered: missing subs or dubs there mean finding a dual-audio or subbed release, though the anime profile already prefers dual-audio so a plain re-grab often does it. Missing audio is never fetchable as a file — it always means a different release. When every provider strikes out, the last resorts are fansub subtitle packs and per-file `subliminal` runs — worked examples in the subtitle references. Real subtitles come from releases, Bazarr/providers, or (only when Andre asks) transcription of the audio itself — **never machine-translate an existing subtitle file into another language**; Andre does not want MTL subs in the library, and a translated-from-English track for a foreign film is worse than no track. Also: any sidecar written into `/data/media` must be world-readable (0644/0664) — a `0660 hermes:hermes` file is invisible to Jellyfin even though ffprobe (running as you) sees it, so "verified present" can be false for viewers.
...
- **Grabbing ONE specific release from a `releases` listing:** `arr <svc> grab '<title>' --override --match '<title substring>'` pushes only the candidates whose name contains the substring (case-insensitive; combine with `--dry-run` to preview). This is the verb for "I picked this exact release" — original-language audio hunts, a specific remux, etc. Never hand-roll `raw POST /release` for it: the GUID-only push QUEUES the job while returning an empty-looking response, so a "retry" double-queues every title (2026-08-14's five duplicate downloads).
